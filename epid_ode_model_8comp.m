function [f,h,hvar,J] = epid_ode_model_8comp(Np)
arguments
    Np = 9709786
end
%%
...
f = {};
f.desc = 'f(x,p,beta,nu,omega)';
f.args = {x_Cas,p_Cas,beta_Cas,nu_Cas,omega_Cas};
f.val = x_Cas + Ts*Fn_SLPIAHDR_ode(x_Cas,p_Cas,beta_Cas,nu_Cas,omega_Cas);
f.Fn = Function('f',f.args,{f.val},{'x','p','beta','nu','omega'},{f.desc});
f.fh = @Fn_SLPIAHDR_ode;
...
f.      Input_3 = beta;
f.desc__Input_3 = 'Transmission rate of the pathogen (beta = TrRate)';
f.      Input_4 = nu;
f.desc__Input_4 = 'Immunity gain rate due to vaccination (nu = ImGainRate)';
f.      Input_5 = omega;
f.desc__Input_5 = 'Immunity loss rate due to waning immunity (omega = ImLossRate)';

h = {};
h.desc = '[all_inf,daily_new_inf,R0,Rt]';
h.args = {x_Cas,p_Cas,beta_Cas};
h.val = Fn_SLPIAHDR_out(x_Cas,p_Cas,beta_Cas);
h.Fn = Function('h',h.args,{h.val},{'x','p','beta'},{h.desc});
h.fh = @Fn_SLPIAHDR_out;

h.      Input_1 = x_Cas;
h.desc__Input_1 = 'State vector (x = [S,L,P,I,A,H,D,R])';
h.      Input_2 = p;
h.desc__Input_2 = 'Parameter vector (p)';
h.      Input_3 = beta;
h.desc__Input_3 = 'Transmission rate of the pathogen (beta = TrRate)';
...
hvar.Fn = [];
vars = [x_Cas;beta_Cas;omega_Cas;p_Cas];
Jh = jacobian(h.val,vars);

[Sigma,Sigma_half] = Pcz_CasADi_Helper.create_sym('Sigma',numel(vars),1,'str','sym');

hvar.val = diag(Jh * Sigma * Jh');
hvar.Fn = Function('hvar',{x_Cas,p_Cas,beta_Cas,omega_Cas,Sigma_half},{hvar.val},{'x','p','beta','omega','Sigma'},{'[Var_all_inf,Var_daily_new_inf,Var_Rc]'});
